...
def add_guest(request):
    ...
    if not result:
        return JsonResponse({'status':10022,'message':'event id error '})
    ...
    ### django datetime日期格式转换成int型时间戳用于比较当前签到时间与发布会时间
...
def user_sign(request):
    ...
    if not result:
        return JsonResponse({'status':'10025','message':'user phone error'})
