...
class LoginActionTest(TestCase):
    ...
    def test_login_action_username_password_null(self):
        ...
        response = self.client.post('/login_action/',data=test_data) #用户名密码错误返回包含错误信息的index.html页
        self.assertEqual(response.status_code,200)
        self.assertIn(b'username or password error',response.content)

    def test_login_action_username_password_error(self):
        '''用户名密码错误'''
        test_data = {'username':'abc','password':'admin123456'}
...
class EventManageTest(TestCase):
    ...
    def setUp(self):
        User.objects.create_user('admin','admin@mail.com','admin123456')
        Event.objects.create(name='xiaomi5',limit=2000,address='beijing',status=True,start_time='2017-8-10 12:30:00')
...
class GuestManageTest(TestCase):
    ...
    def setUp(self):
        ...
        Guest.objects.create(realname='alen',phone=18611001100,email='alen@mail.com',sign=False,event_id=1)
        self.login_user = {'username': 'admin', 'password': 'admin123456'}
    ...
    def test_guest_manage_search_success(self):
        ...
        response = self.client.post('/search_person/',{'name':'alen'})
        print(response)
        self.assertEqual(response.status_code,200)
        self.assertIn(b"alen",response.content)
        self.assertIn(b"18611001100",response.content)
...
class SignIndexActionTest(TestCase):
    ...
    def setUp(self):
        ...
        Event.objects.create(id=2, name="oneplus6", limit=2000, address='shenzhen', status=1,
                             start_time='2017-06-10 12:30:00')
        Guest.objects.create(realname='alen',phone='18611001100',email='alen@mail.com',sign=0,event_id=1)
        Guest.objects.create(realname='mikl', phone='18611001102', email='mike@mail.com', sign=1, event_id=2)
        self.login_user={'username':'admin','password':'admin123456'}
